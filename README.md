...
A Gist template designed for rapid Python development in VSCode. It occupies the middle ground between one-off scripts and full-featured repositories. It includes refactoring tools, optional type checking, linting, and code formatting. You might ask, why use these tools in the first place? See [below](#next-steps-and-template-features) for detail on template features, example usage, and some reasons why you might want to use these features in your development workflow.
...
Now you should be able to modify and run Python scripts in this Gist. Try adding some Python code to the `main()` function of `example.py`. Run your script from the terminal like e.g. `python example.py`, or by clicking the "play button" icon in the VSCode tab ribbon while `example.py` is open. See [next steps and template features](#next-steps-and-template-features) with examples involving the code in `example.py`.
...
[⬆ back](#next-steps-and-template-features)
